...
def test_validate_secret_requires_secret():
	...
	with pytest.raises(ValueError, match="required"):
		tool.handler(
		    _make_invocation(tool, secret_type="some_type")
		)
...
def test_validate_secret_requires_secret_type():
	...
	with pytest.raises(ValueError, match="required"):
		tool.handler(
		    _make_invocation(tool, secret="some_secret")
		)
...
def test_validate_secret_valid(monkeypatch):
	...
	result = tool.handler(
	    _make_invocation(
	        tool, secret="s3cr3t", secret_type="fake"
	    )
	)
	body = json.loads(result["textResultForLlm"])
...
def test_validate_secret_invalid(monkeypatch):
	...
	result = tool.handler(
	    _make_invocation(
	        tool, secret="bad", secret_type="fake"
	    )
	)
	body = json.loads(result["textResultForLlm"])
...
def test_validate_secret_error_in_check(monkeypatch):
	...
	result = tool.handler(
	    _make_invocation(
	        tool, secret="x", secret_type="fake"
	    )
	)
	body = json.loads(result["textResultForLlm"])
...
def test_validate_secret_metadata_failure_preserves_result(
	monkeypatch,
):
	...
	result = tool.handler(
	    _make_invocation(
	        tool, secret="x", secret_type="my_type"
	    )
	)
	body = json.loads(result["textResultForLlm"])
	assert body["status"] == "valid"
